...
typedef RY_keypair *RY_keypair_t;
...
void RY_param_generate(RY_param_t params){
...
void RY_keypair_generate(RY_param* param,RY_keypair_t kpair){
...
static void RY_sig_context_init(
    RY_context* ctx, /* context of RY signature */
    RY_param_t params,
    RY_keypair_t kpair,
    element_t* publickey,
    uint32_t signers,
...
void RY_sig_recv_s(RY_context* ctx,element_t s){
...
void RY_sig_send_s(RY_context* ctx,element_t s){
    /* init s */
...
int RY_sig_finalize(RY_context* ctx,element_t sig){
...
int RY_verify(
    ...
    element_copy(apk,publickey_list[0]);
    for(int i=1;i<signers;i++){
        element_mul(apk,apk,publickey_list[i]);
    }
    /* compute H0(m) */
    element_t H0m;
...
int main(){
    ...
    printf("Hello RY signature!\n");
    char msg[] = "helloRY!";
    uint32_t len = sizeof(msg);
    RY_param param;
    RY_context ctx[SIGNERS];
    RY_keypair kpairs[SIGNERS];
    element_t public_keylist[SIGNERS];
    element_t s[SIGNERS];
    element_t sig[SIGNERS];

    /* random generate param */
    RY_param_generate(&param);
    
    /* generate private key & public key */
    for(int i=0;i<SIGNERS;i++){
        RY_keypair_generate(&param,&(kpairs[i]));
    }
    ...
    for(int i=0;i<SIGNERS;i++){
        RY_sig_context_init(&(ctx[i]),&param,&(kpairs[i]),public_keylist,SIGNERS,msg,len);
    }

    /* collect all s from all signers */
    for(int i=0;i<SIGNERS;i++){
        RY_sig_send_s(&(ctx[i]),s[i]);
    }

    /* recv s for all signers */
    for(int i=0;i<SIGNERS;i++){
        for(int j=0;j<SIGNERS;j++){
            RY_sig_recv_s(&(ctx[i]),s[j]);
        }
    }

    /* finalize all signature */
    for(int i=0;i<SIGNERS;i++){
        RY_sig_finalize(&ctx[i],sig[i]);
    }
    ...
    if(RY_verify(&param,public_keylist,SIGNERS,sig[0],msg,len)==0){
